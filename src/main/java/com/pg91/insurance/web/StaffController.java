...
public class StaffController {
    ...
    // Create new staff member using Factory Pattern
    @PostMapping("/create")
    public String createStaff(@Valid @ModelAttribute StaffDTO staffDTO,
        ...
        try {
            // Use the factory method to create staff from DTO
            staffService.createStaffFromDTO(staffDTO);
    ...
    public String viewStaffByRole(@PathVariable String role, Model model) {
        try {
            List<AppUser> staffList = staffService.getStaffByRole(role);
            model.addAttribute("staffList", staffList);
            model.addAttribute("role", role);
            return "admin/staff-by-role";
        } catch (Exception e) {
            model.addAttribute("errorMessage", e.getMessage());
            return "redirect:/admin/staff";
        }
